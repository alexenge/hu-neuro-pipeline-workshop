...
library(here)
knitr::include_graphics(here("slides/figures/venn.png"))
```
...
knitr::include_graphics(here("slides/figures/reticulate.png"))
```
...
- Ocular correction:
    ...
    - Name of an ICA method (default: `"fastica"`)
...
library(tidyverse)
trials <- read_csv("output/trials.csv") # res[[1]]
print(trials)
```
...
# Check single trial N2 mean amplitudes
summary(trials$N2)
...
```{r, eval=FALSE, out.width="50%", warning=FALSE}
# Plot single trial N2 mean amplitudes
trials %>%
  ggplot(aes(x = n_b, y = N2, color = DeviantPosRL)) +
  geom_point(position = position_jitterdodge(0.3), alpha = 0.01) +
  stat_summary(
    geom = "pointrange",
    fun.data = mean_se,
