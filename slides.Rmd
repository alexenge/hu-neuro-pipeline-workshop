...
- Ocular correction with BESA: Directory path or list of BESA files (`.matrix`)

- Alternatively: independent component analysis (e.g., `tfr_method = "fastica"`)
...
- Reject bad epochs (`reject_peak_to_peak = 200.0`)

  - Using per-channel peak-to-peak amplitudes

- Ocular correction (`ica_method = "fastica"`)

  - FastICA [@hyvarinen1999] + correlation with HEOG/VEOG

  - Can specify different methods + number of principal components (via `ica_n_components`)
